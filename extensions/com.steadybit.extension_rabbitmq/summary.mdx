...
Integration of RabbitMQ into Steadybit works via the official [RabbitMQ Management Plugin](https://www.rabbitmq.com/management.html) and its [HTTP API](https://www.rabbitmq.com/docs/http-api-reference), using the [rabbithole](https://github.com/michaelklishin/rabbit-hole) Go client library.

The `extension-rabbitmq` leverages these APIs and AMQP connections to:
- Discover clusters, nodes, vhosts, queues and exchanges
...
- **Exchanges**, along with their type (fanout, topic, direct, headers), durability and auto-delete flag — the default exchange, `amq.*` built-ins and internal exchanges are excluded
...
Sends messages through an exchange at a fixed rate or in fixed quantity — delivery follows the exchange type and its bindings, so you can test fanout distribution and topic routing selectivity. Unroutable messages count as failures.
