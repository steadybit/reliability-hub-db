...
To simulate the unavailability, we can blackhole the PostgreSQL database client connection using the hostname `db.internal.steadybit.com` so that no incoming or outgoing traffic is going through.
