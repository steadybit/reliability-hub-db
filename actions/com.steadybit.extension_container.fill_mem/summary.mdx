...
| Size             | How much of the *total container memory* should be used to stress the memory? The used memory percentage is calculated based on the memory limit of the container (or the host memory if no limit is set).                                    | 80%                           |
| Unit             | Unit for the size parameter. Options: *Megabytes*, *% of total memory*                                                                                                                                                                        | % of total memory             |
