# Introduction

This action leverages [https://github.com/gunnargrosch/failure-lambda](gunnargrosch/failure-lambda) and the AWS Systems Manager API to deny connections to other hosts for an AWS Lambda.

# Use Cases

- Understand how a lambda behaves when an external dependency is unavailable.


# Prerequisites

The AWS Lambda Function to attack must [be wrapped with the failure-lambda library](https://github.com/gunnargrosch/failure-lambda#how-to-install-with-parameter-in-ssm-parameter-store).

# Parameters

| Parameter | Description                                                                       | Default |
|-----------|-----------------------------------------------------------------------------------|---------|
| Duration  | The duration of the attack.                                                       | 30s     |
| Rate      | The rate of invocations to affect. (100 = all request, 50 = half of all requests) | 100%    |
| Deny List | List of regular expressions to match the hosts against                            |         |
