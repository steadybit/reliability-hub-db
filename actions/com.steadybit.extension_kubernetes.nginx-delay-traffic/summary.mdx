...
| Delay                  | The delay to apply to the path in milliseconds.                                                                                                  | 500ms   |
| Path Pattern           | (optional) A regex pattern to match the paths to delay.                                                                                          |         |
| HTTP Method            | (optional) A http method to match the requests to delay. * is used as not set.                                                                   | *       |
| HTTP Header            | (optional) A list of HTTP headers to match the requests to delay. Value is a regex pattern. If multiple headers are set, all of them must match. |         |
