...
| Mode                                                               | Specify how to calculate the amount of disk space to fill.: <br/> *Fill up to specified usage (in %)*, <br/> *Megabytes to write*,<br/> *Megabytes to leave free on disk*                        | Fill up to specified usage (in %)(*PERCENT*) |
| Block Size (in MBytes) of the File to Write for method `Over Time` | (optional) Only applicable for method over time (`dd`): The block size when writing the file. If the block size is larger than the size value, the attack uses the size value as the block size. | 5                                            |
| Method used to fill disk                                           | Should the disk be filled all at once (using `fallocate`) or gradually over time (using `dd`)?                                                                                                   | AT_ONCE                                      |
| Fill Value (depending on Mode)                                     | Depending on the mode, specify the percentage or megabytes to use.                                                                                                                               | 500                                          |
| File Destination                                                   | Where to temporarily write the file for filling the disk. It will be cleaned up afterwards.                                                                                                      | /tmp                                         |
| Fail on OOM Kill                                                   | (optional) Should an OOM kill be considered a failure?                                                                                                                                           | true                                         |

# Caveats

## The underlying disk is (most likely) shared

The root filesystem of a container typically uses an overlay filesystem that is backed by the host's disk. That means that all containers running on the same host share the same underlying disk.
Thus, if you run multiple instances of this attack on the same host, they will all write to the same disk.
The Kubernetes ephemeral storage limit for a container doesn't prevent writes from exceeding the limit, but it will cause Kubernetes to kill the container.
Therefore, the modes *Fill up to specified usage (in %)* and *Megabytes to leave free on disk* will calculate **depending on the total size of the disk (backing the container root fs), and not based on the ephemeral storage limit**.
...
## Q: I get a "Operation not permitted" error when running the attack. Why?
**A:** Some underlying filesystems (e.g., `ZFS`) do not support preallocation, which is used in the 'at once' mode. In such cases, you can switch to the 'over time' mode, which uses the `dd` command and should work on all filesystems.