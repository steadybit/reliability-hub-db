...
| Mode       | Specify how to calculate the amount of disk space to fill.: <br/> *Fill up to specified usage (in %)*, <br/> *Megabytes to write*,<br/> *Megabytes to leave free on disk*             | overall percentage of filled disk space in percent (*PERCENT*) |
...
The root filesystem of a container is typically using an overlaying fs which is backed by the host's disk. This means that all containers running on the same host share the same underlying disk.
Thus, if you run multiple instances of this attack on the same host, they will all write to the same disk.
The Kubernetes' ephemeral storage limit for a container doesn't prevent the writes exceeding the limit, but will cause the container to be killed.
Thus the modes *Fill up to specified usage (in %)* and *Megabytes to leave free on disk* will calculate **depending on total size of the disk (backing the container root fs), and not based on the ephemeral storage limit**.
