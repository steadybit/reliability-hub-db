...
| Expected Status List | (optional) The expected state of the monitor. One of Ok, Alert, Warn, No Data, Unknown, Skipped, Ignored.     |                |
| Status Check Mode    | How often should the status be expected. "At least once" or "All the time"                                    | "All the time" |
| Multi Alert Filter   | (optional) Filter to only consider alerts for specific groups of a multi alert. Specified as key-value pairs. |                |