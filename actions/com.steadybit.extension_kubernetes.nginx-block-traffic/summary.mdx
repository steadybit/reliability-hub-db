...
| Status Code            | The status code which should get returned (100-999).                                                                                             | 503     |
| Path Pattern           | (optional) A regex pattern to match the paths to block.                                                                                          |         |
| HTTP Method            | (optional) A http method to match the requests to block. * is used as not set.                                                                   | *       |
| HTTP Header            | (optional) A list of HTTP headers to match the requests to block. Value is a regex pattern. If multiple headers are set, all of them must match. |         |
| Force Enterprise NGINX | (optional) Whether to use Enterprise NGINX configuration instead of open source.                                                                 | false   |