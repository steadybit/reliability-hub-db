...
Fill the host's hard disk with random data for a given duration.
...
### When should you use the 'at once' mode?
The mode is perfect if you want to check how your system behaves when the disk is almost full, and you don't need to simulate a gradual decrease in free space.
...
### When should you use the 'over time' mode?
The over-time approach is perfect when you want to simulate a more realistic case of a gradually filling disk or need to fill the disk more than available space.
...
**A:** Use the following command to check if the disk is full:
```
df -h
```
